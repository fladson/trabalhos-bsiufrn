...
Laboratório de Gerenciamento de Memória

    Fladson Thiago Oliveira Gomes
    Kelvin Kirk Miná Cavalcante
*/
...
selectVictimFifo (void)
{
  ...
  while(i < n_frames){
      if(frameTable[i].load_time < frameTable[victim].load_time){
          victim = i;
      }
      i++;
  }
...
selectVictimLRU (void)
{
  ...
  int i =1;
  while(i < n_frames){
      if(frameTable[i].last_reference_time < frameTable[victim].last_reference_time){
            victim = i;
      }
      i++;
  }
...
simulateOneStep (unsigned int addr, char rw)
{
  ...
  if (!pageTable[page].valid) {
	...
	/* actualiza tabela de frames
	 note-se que o passo da simulacao em que se esta
...
main (int argc, char *argv[])
{
  ...
  if (argc != 4) {		/* Garantir que foram passados 1+3
    ...
    'algoritmo'         pode ser um de \"r[andom]\", \"f[ifo]\" ou \"l[ru]\"\n\n",
		execname);
    exit (1);
  }
